...
Usage: /af-fox https://github.com/{owner}/{repo}/issues/{number}

Example: /af-fox https://github.com/acme/widgets/issues/42
```
...
Print progress:
```
[af-fox] Fetching issue #{number} from {owner}/{repo}...
```
...
Print progress:
```
[af-fox] Analyzing issue #{number}: {title}...
```
...
Print progress:
```
[af-fox] Posting analysis to issue #{number}...
```
...
> Auto-generated by `af-fox`. Reviewing issue context and codebase.
...
---
*Analysis by `af-fox`. Implementation follows.*
```
...
Print:
```
[af-fox] Created branch {branch_name}
```
...
Print progress:
```
[af-fox] Implementing fix for issue #{number}...
```
...
Print progress:
```
[af-fox] Posting fix summary to issue #{number}...
```
...
---
*Fix by `af-fox`. Ready for review.*
```
...
Commit the changes and merge into `develop`.
...
The `(fixes #{number})` trailer links the commit to the issue.
...
Log each retry:
```
[af-fox] Push failed, retrying in {delay}s (attempt {n}/3)...
```
...
```
[af-fox] ✅ Issue #{number} fixed and merged to develop.
  Issue:    {title}
  Branch:   {branch_name}
  Commit:   {commit_hash}
...
This completes the af-fox workflow. Do not perform any additional actions after
