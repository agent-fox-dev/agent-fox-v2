...
if HAS_HYPOTHESIS:
    ...
    @given(
        ...
        archetype=st.text(max_size=50),
    )
    @settings(max_examples=50)
    def test_extraction_stub_safety(
        session_id: str,
        transcript: str,
        spec_name: str,
        archetype: str,
    ) -> None:
        """TS-100-P3: For any ExtractionInput, extract_knowledge never raises.

        The stub must always return a valid ExtractionResult with
        status='not_implemented' regardless of input content.
        """
    ...
    def test_extraction_stub_safety_no_hypothesis() -> None:  # type: ignore[misc]
        """TS-100-P3: Extraction stub safety without Hypothesis."""
        from agent_fox.nightshift.extraction import (
            ExtractionInput,
            ExtractionResult,
            extract_knowledge,
        ...
        test_cases = [
            ("", "", "", ""),
            ("s1", "long transcript " * 100, "spec", "coder"),
            ("session\n42", "content with\nnewlines", "my-spec", "maintainer"),
        ]
        for session_id, transcript, spec_name, archetype in test_cases:
            inp = ExtractionInput(
                session_id=session_id,
                transcript=transcript,
                spec_name=spec_name,
