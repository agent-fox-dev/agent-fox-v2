...
class TestPreReviewModeConfig:
    ...
    def test_pre_review_config(self) -> None:
        ...
        assert cfg.default_allowlist == [], (
            f"pre-review allowlist should be empty, got {cfg.default_allowlist}"
        )
        assert cfg.injection == "auto_pre", (
            f"pre-review injection should be 'auto_pre', got {cfg.injection!r}"
        )
...
class TestDriftReviewModeConfig:
    ...
    def test_drift_review_config(self) -> None:
        ...
        assert cfg.injection == "auto_pre", (
            f"drift-review injection should be 'auto_pre', got {cfg.injection!r}"
        )
        assert cfg.default_model_tier == "STANDARD", (
...
class TestAuditReviewModeConfig:
    ...
    def test_audit_review_config(self) -> None:
        ...
        assert "uv" in cfg.default_allowlist, (
            f"'uv' must be in audit-review allowlist, got {cfg.default_allowlist}"
        )
        assert cfg.injection == "auto_mid", (
            f"audit-review injection should be 'auto_mid', got {cfg.injection!r}"
        )
...
class TestFixReviewModeConfig:
    ...
    def test_fix_review_config(self) -> None:
        ...
        assert cfg.injection is None, (
            f"fix-review injection should be None (no auto-injection), got {cfg.injection!r}"
        )
        assert cfg.default_allowlist is not None, "fix-review allowlist must not be None"
        assert "make" in cfg.default_allowlist, (
            f"'make' must be in fix-review allowlist, got {cfg.default_allowlist}"
        )
...
class TestCoderFixMode:
    ...
    def test_coder_fix_mode(self) -> None:
        ...
        assert "fix" in entry.modes, (
            f"coder should have 'fix' mode, got modes: {set(entry.modes.keys())}"
        )
...
class TestVerifierSingleInstance:
    ...
    def test_verifier_single(self) -> None:
        ...
        assert clamp_instances("verifier", 3) == 1, (
            "verifier instances should be clamped to 1, got "
            f"{clamp_instances('verifier', 3)}"
        )
    ...
    def test_verifier_single_large(self) -> None:
        ...
        assert clamp_instances("verifier", 100) == 1, (
            "verifier instances should always be clamped to 1"
        )
    ...
    def test_instances_config_verifier_default(self) -> None:
        ...
        assert hasattr(cfg, "reviewer"), (
            "ArchetypeInstancesConfig should have 'reviewer' field"
        )
        assert cfg.verifier == 1, (
            f"ArchetypeInstancesConfig.verifier default should be 1, got {cfg.verifier}"
        )
...
class TestOldEntriesRemoved:
    ...
    def test_old_removed(self, name: str) -> None:
        ...
        assert name not in ARCHETYPE_REGISTRY, (
            ...
            f"during reviewer consolidation"
        )
    ...
    def test_old_fallback(self) -> None:
        ...
        assert result == ARCHETYPE_REGISTRY["coder"], (
            "get_archetype('skeptic') should fall back to coder entry"
        )
...
class TestConfigReviewerToggle:
    ...
    def test_config_toggle(self) -> None:
        ...
        assert hasattr(cfg, "reviewer"), (
            "ArchetypesConfig should have 'reviewer' toggle"
        )
        assert cfg.reviewer is True, (
            f"ArchetypesConfig.reviewer default should be True, got {cfg.reviewer}"
        )
        # Old toggles should be removed
        assert not hasattr(cfg, "skeptic"), (
            "ArchetypesConfig should not have 'skeptic' field after consolidation"
        )
        assert not hasattr(cfg, "oracle"), (
            "ArchetypesConfig should not have 'oracle' field after consolidation"
        )
        assert not hasattr(cfg, "auditor"), (
            "ArchetypesConfig should not have 'auditor' field after consolidation"
        )
...
class TestReviewerConfig:
    ...
    def test_reviewer_config(self) -> None:
        ...
        assert rc.drift_review_block_threshold is None, (
            f"drift_review_block_threshold should be None (advisory), "
            f"got {rc.drift_review_block_threshold}"
        )
        assert rc.audit_min_ts_entries == 5, (
            f"audit_min_ts_entries should be 5, got {rc.audit_min_ts_entries}"
        )
        assert rc.audit_max_retries == 2, (
            f"audit_max_retries should be 2, got {rc.audit_max_retries}"
        )
    ...
    def test_reviewer_config_in_archetypes_config(self) -> None:
        ...
        cfg = ArchetypesConfig()
        assert hasattr(cfg, "reviewer_config"), (
            "ArchetypesConfig should have 'reviewer_config' field"
        )
...
class TestCoderWithoutMode:
    ...
    def test_coder_no_mode(self) -> None:
        ...
        assert cfg.templates == ["coding.md"], (
            f"coder default templates should be ['coding.md'], got {cfg.templates}"
        )
        assert cfg.default_max_turns == 300, (
            f"coder default max_turns should be 300, got {cfg.default_max_turns}"
        )
...
class TestReviewerDisabled:
    ...
    def test_reviewer_disabled(self) -> None:
        ...
        assert len(reviewer_entries) == 0, (
            f"Expected no reviewer entries when reviewer=False, got {reviewer_entries}"
        )
